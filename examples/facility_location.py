...
if not ship_df.empty:
    ...
    ship_df = ship_df.merge(
        transport_df,
        left_on=['facilities', 'customers'],
        right_on=['facility', 'customer']
    )
...
if not ship_df.empty:
    facility_summary = ship_df.groupby('facilities').agg({
        'units': 'sum',
        'shipping_cost': 'sum'
    }).round(1)
...
if not ship_df.empty:
    customer_summary = ship_df.groupby('customers').agg({
        'units': 'sum',
        'shipping_cost': 'sum'
    }).round(1)
